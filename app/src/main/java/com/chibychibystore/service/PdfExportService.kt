            ...
            when (reportData) {
                is Result.Success -> {
                    ...
                    val filePath = withContext(ioDispatcher) {
                        val path = createPdfFile(fileName)
                        createGrossSalesPdf(path, reportData.data, startDate, endDate)
                        path
                    }
        ...
        return try {
            ...
            when (reportData) {
                is Result.Success -> {
                    ...
                    val filePath = withContext(ioDispatcher) {
                        val path = createPdfFile(fileName)
                        createProfitMarginPdf(path, reportData.data, startDate, endDate)
                        path
                    }
                    Result.success(filePath)
                }
        ...
        return try {
            ...
            when (reportData) {
                is Result.Success -> {
                    ...
                    val filePath = withContext(ioDispatcher) {
                        val path = createPdfFile(fileName)
                        createNetProfitPdf(path, reportData.data, startDate, endDate)
                        path
                    }
                    Result.success(filePath)
                }
        ...
        return try {
            ...
            when (reportData) {
                is Result.Success -> {
                    ...
                    val filePath = withContext(ioDispatcher) {
                        val path = createPdfFile(fileName)
                        createSalesByProductPdf(path, reportData.data, startDate, endDate)
                        path
                    }
                    Result.success(filePath)
                }
        ...
        return try {
            ...
            when (reportData) {
                is Result.Success -> {
                    ...
                    val filePath = withContext(ioDispatcher) {
                        val path = createPdfFile(fileName)
                        createSalesByCategoryPdf(path, reportData.data, startDate, endDate)
                        path
                    }
                    Result.success(filePath)
                }
        ...
        return try {
            ...
            when (reportData) {
                is Result.Success -> {
                    ...
                    val filePath = withContext(ioDispatcher) {
                        val path = createPdfFile(fileName)
                        createSalesTrendPdf(path, reportData.data, startDate, endDate)
                        path
                    }
                    Result.success(filePath)
                }
        ...
        return try {
            ...
            when (reportData) {
                is Result.Success -> {
                    ...
                    val filePath = withContext(ioDispatcher) {
                        val path = createPdfFile(fileName)
                        createIncomeStatementPdf(path, reportData.data, date)
                        path
                    }
                    Result.success(filePath)
                }
        ...
        return try {
            ...
            when (reportData) {
                is Result.Success -> {
                    ...
                    val filePath = withContext(ioDispatcher) {
                        val path = createPdfFile(fileName)
                        createCashFlowPdf(path, reportData.data, startDate, endDate)
                        path
                    }
                    Result.success(filePath)
                }
        ...
        return try {
            ...
            when (reportData) {
                is Result.Success -> {
                    ...
                    val filePath = withContext(ioDispatcher) {
                        val path = createPdfFile(fileName)
                        createExpenseReportPdf(path, reportData.data, startDate, endDate)
                        path
                    }
                    Result.success(filePath)
                }
        ...
        return try {
            ...
            when (reportData) {
                is Result.Success -> {
                    ...
                    val filePath = withContext(ioDispatcher) {
                        val path = createPdfFile(fileName)
                        createBalanceSheetPdf(path, reportData.data, asOfDate)
                        path
                    }
                    Result.success(filePath)
                }
