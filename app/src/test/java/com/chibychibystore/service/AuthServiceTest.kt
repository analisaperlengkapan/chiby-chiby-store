...
class AuthServiceTest {
    private lateinit var mockDao: PenggunaDao
    private lateinit var mockSessionRepo: PenggunaSessionRepository
    private lateinit var service: AuthService
    @Before
    fun setup() {
        mockDao = mock()
        mockSessionRepo = mock()
        service = AuthServiceImpl(mockDao, mockSessionRepo)
    }
    @Test
    fun loginSuccess() = runTest {
        ...
        whenever(mockDao.getPenggunaByUsername("u")).thenReturn(user)
        whenever(mockSessionRepo.createSession(any())).thenReturn(Result.success(1L))
        val res = service.login("u", pw)
